...
router
    ...
    try {
        if (ImageIndex === 0) {
            const { rows: thumbnail } = yield (0, database_1.query)(QueryString.CheckThumbnail(), [product_uid], {
                privileges,
                actions: [constants_1.CREATE],
            });
            if ((_a = thumbnail[0]) === null || _a === void 0 ? void 0 : _a.thumbnail) {
        ...
        const { rows: product } = yield (0, database_1.query)(`SELECT title from products WHERE product_uid = $1`, [product_uid], {
            privileges,
            actions: [constants_1.CREATE],
        });
        if (!((_b = product[0]) === null || _b === void 0 ? void 0 : _b.title)) {
            return res.status(400).json({
                success: false,
                error: { message: 'Product title does not exist!' },
            });
        }
        const { image, error } = yield (0, upload_1.default)(url, (_c = product[0]) === null || _c === void 0 ? void 0 : _c.title);
        if (error) {
            return res.status(500).json({ success: false, error });
        }
        yield (0, database_1.query)(QueryString.InsertImage(), [product_uid, image.path, ImageIndex === 0, ImageIndex], {
            privileges,
            actions: [constants_1.CREATE],
        });
    ...
    if (!image_uid)
        return res.status(403).json({ success: false, error: 'Require Fields!' });
    try {
        (0, S3_1.deleteObject)(image_uid, (error) => __awaiter(void 0, void 0, void 0, function* () {
            if (error) {
                return res.status(500).json({ success: false, error });
            }
            yield (0, database_1.query)(QueryString.DeleteImage(), [image_uid], {
                privileges,
