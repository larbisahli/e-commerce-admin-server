...
async function query<T, P>(text: string, params: P[], {privileges, actions}:QueryPermissionType): Promise<QueryResult<T>> {
  const HasPrivileges = actions.every((action) => privileges?.includes(action))
  if(!HasPrivileges) throw Error("Permission Denied.")
  
  switch (actions[0]) {
      case READ: return await ReadPool.query(text, params)
      case CREATE: return await CreatePool.query(text, params)
      case UPDATE: return await UpdatePool.query(text, params)
      case DELETE: return await DeletePool.query(text, params)
      default: throw Error("Permission Denied.")
    }
}

async function getClient({privileges, actions}:QueryPermissionType): Promise<PoolClient> {
